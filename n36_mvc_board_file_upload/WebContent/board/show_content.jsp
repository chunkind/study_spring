...
<script type="text/javascript">
	function deleteBoard(){
		if(confirm("해당 글을 삭제 하시겠습니까?")){
			location.href="board.do?command=delete&&no=${requestScope.bvo.no}&&filename=${bvo.newfilename}";
		}
	}
		...
		<tr>
		   <td>
		   	    <table width="550">
					...
					<!-- orgFileName 이름을 가지고 오면 된다.
						orgFileName이 null이 아니라면
					 -->
					<c:if test="${bvo.orgfilename!=null}">
						<tr>
							<td>
								<a href="board.do?command=fileDown&&filename=${bvo.newfilename}"><b>${bvo.orgfilename}</b></a>
							</td>
						</tr>
					</c:if>	
					...
					<tr>
						<td valign="middle">
						<a href="board.do?command=list&pageNo=1">
						<img alt="전체글목록" src="img/list_btn.jpg">
						</a>
